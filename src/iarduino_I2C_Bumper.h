...
class iarduino_I2C_Bumper{																													//
	...
	/**	Пользовательские функции **/																										//
		...
		float					getSidePID				(int8_t							);													//	Объявляем  функцию возвращающую ошибку края  для ПИД регулятора		(Параметр:  любое число, знак которого определяет край линии - левый, + правый. Функцию удобно использовать для маневрирования на перекрёстках.
		bool					getCross				(uint8_t, uint32_t=0			);													//	Объявляем  функцию обнаружения перекрёстка или крутого поворота		(Параметры: толщина линии трассы (количество датчиков над линией), [время в мс требуемое для преодоления перекрёстка]).
