...
class iarduino_I2C_Bumper{																													//
	...
	/**	Пользовательские функции **/																										//
		...
		bool					setTurnSignal			(uint8_t mode					);													//	Объявляем  функцию управления поворотниками							(Параметры: режим работы поворотников).
		bool					setTurnPeriod			(uint8_t period					);													//	Объявляем  функцию установки периода мигания поворотников			(Параметр:  период).
		bool					settingsTurnAuto		(uint8_t, uint8_t, bool=false	);													//	Объявляем  функцию настройки работы поворотников в авто-режиме		(Параметры: условие включения, условие отключения, [добавить 2 мигания перед отключением true/false]).
		...
		bool					setCalibrationManual	(uint16_t i						){return setCalibrationManual(i,i,i,i,i,i,i,i,i);}	//	Определяем функцию запуска ручной калибровки модуля					(Параметры: граница АЦП между линией и фоном для всех датчиков модуля).
		...
		uint16_t				getLineDigital			(uint8_t num					);													//	Объявляем  функцию возвращающую флаг наличия линии под датчиком		(Параметр:  номер датчика).
		uint16_t				getLineAnalog			(uint8_t num					);													//	Объявляем  функцию возвращающую значение АЦП снятое с датчика		(Параметр:  номер датчика).
		float					getErrPID				(void							);													//	Объявляем  функцию возвращающую ошибку цнтра для ПИД регулятора		(Параметр:  отсутствует). Функцию удобно использовать для движения по линии любой сложности.
...
template<typename T> uint8_t	getLineSum				(T &a){uint8_t i,j=0; a=(T)getLineDigital(BUM_LINE_ALL); for(i=1;i<=9;i++){j+=a&bit(i)?1:0;} return j;} //													(Параметр:  переменная для получения флагов наличия линий).
		uint8_t					getLineSum				(void							){ uint16_t a=0; return getLineSum(a); }			//	Определяем функцию возвращающую количество датчиков над линией		(Параметр:  отсутствует).
