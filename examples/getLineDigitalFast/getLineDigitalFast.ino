...
void setup(){                                              //
    ...
    bum.begin();                                           //   Инициируем работу с бампером.
                                                           ...
                                                           //
void loop(){                                               //
    uint8_t i = bum.getLineDigital(BUM_LINE_ALL);          //   Читаем состояние сразу всех датчиков. Каждый бит переменной i отвечает за свой датчик (кроме 0 бита).
    ...
    delay(200);                                            // * Задержка позволяет медленнее заполнять монитор последовательного порта.
                                                           ...
                                                           //
/*  Данный пример, в отличии от примера «getLineDigital», читает состояние всех датчиков одним байтом,
